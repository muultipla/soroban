(function(){
  ...
  function pushClass(el, targetClass) {
    ...
    if (el && typeof el == 'object' && targetClass) {
      ...
      if (!elClass.contains(targetClass)) {
  ...
  function containsClass(el, targetClass) {
    let ret = false;
    if (el && typeof el == 'object' && targetClass) {
      ret = el.classList.contains(targetClass);
    }
  ...
  (function() {
    ...
    function toggleMenu(){
      ...
      let status = !!(menuOpen || menuPulled);
    ...
    if(links) {
      Array.from(links).forEach(function(link){
        ...
        if(isExternal) {
          ...
          if (attr1) {
            elemAttribute(link, target, blank);
          }
          if (attr2) {
            elemAttribute(link, rel, noopener);
          }
        }
      });
    }
